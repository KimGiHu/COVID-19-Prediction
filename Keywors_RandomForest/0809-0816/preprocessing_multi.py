...
store_path = os.path.join(current_path,'python_TF-IDF_%s'%args.lang) # stored data directory
...
table.to_csv(store_path +'/keywords_noun_%s.csv'%args.count)
...
######################################################
keywords = pd.read_csv(store_path+'/keywords_noun_%s.csv'%args.count)['word'].tolist()

# keywords: list
# searching: list.index(item)
createDirectory(os.path.join(store_path,'dvectors_noun'))
createDirectory(os.path.join(store_path,'vectors2_noun'))
...
for date in dates:
    ...
    np.savetxt(os.path.join(store_path,'dvectors_noun')+'/%s.csv'%date, doc_vectors, fmt = '%d', delimiter=',')
    np.savetxt(os.path.join(store_path,'vectors2_noun')+'/%s.csv'%date, total_data, fmt = '%d', delimiter=',')

doc_vectors = 0
   
for date in val_dates:
    doc_vector = np.loadtxt(os.path.join(store_path,'dvectors_noun')+'/%s.csv'%date, delimiter=',')
