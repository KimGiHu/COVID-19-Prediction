...
parser.add_argument('--alpah', type=int,
                    default=0.00015, help='Choose your vocab size')
...
for date in train_dates:
    # vector = np.loadtxt(path+'/vectors2/%s.csv'%date, delimiter=',') # preprocessing not only nouns but also verbs, adjectives, adverbs
    if (args.lang == 'Republic of Korea'):
        vector = np.loadtxt(path+'/vectors2_noun/vectors2%s.csv'%date, delimiter=',') # preprocessing only nouns
    ...
    if date.replace('_','-') in key_date[key_date['label']==1]['date'].tolist():
        ...
        # key_vector.append(vector/len(news_len))
    else:
        non_key_vector.append(vector)
        # non_key_vector.append(vector/len(news_len))
...
soykeyword_metric1 = proportion_key/ ((proportion_key+proportion_non_key+1e-8) * np.sum(np.sum(key_vector, axis=0)+np.sum(non_key_vector, axis=0)))
soykeyword_metric2 = appearance_key/ (appearance_key+appearance_non_key+1e-8)
# soykeyword_metric3 = average_key / ((average_key + average_non_key+1e-8) * np.average((np.sum(key_vector,axis=0)+np.sum(non_key_vector,axis=0))))
...
# baseline Method
# freq_metric1 = np.tanh()
freq_metric1 = np.tanh(30000 * key_freq/(np.sum(total_freq)+1e-8))
freq_metric2 = np.tanh(0.00015 * key_freq)
freq_metric3 = key_freq/(np.sum(total_freq)+1e-8)
# freq_metric3 = np.tanh(0.00015 * np.average(key_vector, axis=0)/ np.average((np.sum(key_vector,axis=0)+np.sum(non_key_vector,axis=0))) )

# freq_metric3 = np.tanh(0.015* np.average(key_vector,axis=0))
# freq_metric3 = np.tanh(np.sum(key_vector,axis=0)/np.sum(np.sum(key_vector,axis=0)+np.sum(non_key_vector,axis=0)))

# proposed method1 on rev_metric1 & rev_metric3
# freq_metric = np.zeros(len(total_freq))
# for j in range(len(total_freq)):
#     freq_metric[j] = np.tanh(np.sum(key_vector,axis=0)[j]/total_freq[j])

# freq_metric = np.tanh(key_freq/(total_freq))

# proposed method2 : eleminate the tanh func.
# freq_metric = key_freq * args.alpah

rev_metric1 = soykeyword_metric2 * freq_metric1
rev_metric2 = soykeyword_metric2 * freq_metric2
rev_metric3 = soykeyword_metric2 * freq_metric3
