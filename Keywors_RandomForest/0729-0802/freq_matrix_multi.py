...
keywords = pd.read_csv(path + '/keywords_%s.csv'%args.count)['word'].tolist()
...
words = pd.read_csv(path+'/keywords_%s.csv'%args.count)['word'].tolist()
...
createDirectory(path+'/keywords2000')
pd.DataFrame(data_metric).to_csv('./python_TF-IDF_%s'%args.lang+'/keywords2000/metric_for_paper_%s.csv'%args.count, index=False)

# to form the total matrix of dates : keyword
complete_matrix = {}

for date in dates:
    vector = pd.read_csv(path+'/vectors2/%s.csv'%date, header=None).values.flatten()
    complete_matrix[date] = vector

complete_table = pd.DataFrame(complete_matrix, index=keywords)
complete_table.to_csv(path+'/keywords2000/complete_table_%s.csv'%args.count)
...
for name in names:
    data_metric = pd.DataFrame(pd.read_csv('./python_TF-IDF_%s'%args.lang+'/keywords2000/metric_for_paper_%s.csv'%args.count))
    data_metric.sort_values(by=[name], ascending=False, inplace=True)
    keywords = data_metric['keywords'].tolist()
    pd.DataFrame(keywords).to_csv('./python_TF-IDF_%s'%args.lang+'/keywords2000/%s.csv'%name, index=False, header=False)
    total_data = pd.read_csv('./python_TF-IDF_%s'%args.lang+'/keywords2000/complete_table_%s.csv'%args.count, index_col='Unnamed: 0').T[keywords[:2000]]
    total_data.to_csv('./python_TF-IDF_%s'%args.lang+'/keywords2000/frequency_matrix_%s'%args.count+'_%s.csv'%name)
...
words = pd.read_csv('./python_TF-IDF_%s'%args.lang+'/keywords_%s.csv'%args.count)['word'].tolist()
...
pd.DataFrame(data_metric).to_csv('./python_TF-IDF_%s'%args.lang+'/keywords2000/metric_for_paper_ctfidf_%s.csv'%args.count, index=False)

for name in ['ctfidf']:
    data_metric = pd.DataFrame(pd.read_csv('./python_TF-IDF_%s'%args.lang+'/keywords2000/metric_for_paper_ctfidf_%s.csv'%args.count))
    data_metric.sort_values(by=[name], ascending=False, inplace=True)
