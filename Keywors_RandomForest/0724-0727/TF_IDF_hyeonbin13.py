...
pd.DataFrame(data_metric).to_csv('./python_TF-IDF/metric_for_paper_200.csv', index=False)

# to form the total matrix of dates : keyword
complete_matrix = {}

for date in dates:
    vector = pd.read_csv('./python_TF-IDF/vectors2_200/%s.csv'%date, header=None).values.flatten()
    complete_matrix[date] = vector

complete_table = pd.DataFrame(complete_matrix, index=keywords)
complete_table.to_csv('./python_TF-IDF/complete_table_200.csv')
...
for name in names:
    data_metric = pd.DataFrame(pd.read_csv('./python_TF-IDF/metric_for_paper_200.csv'))
