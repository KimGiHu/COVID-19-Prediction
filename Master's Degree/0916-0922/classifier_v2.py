...
names = ['ctfidf'] + ['rev_metric1', 'rev_metric2', 'rev_metric3','rev_metric4']
...
for name in names:
    ...
    for N in Ns:
        ...
        for (train_ids, test_ids) in kfold.split(train_dataset[0]):
            ...
            if name=='rev_metric4':
                clf4 = RandomForestClassifier(n_estimators=100, max_depth=50, n_jobs=-1, class_weight='balanced_subsample', random_state=0)

                clf4.fit(train_X, train_y)
                test_pred_cv = clf4.predict_proba(test_X_cv)[:,1]
            # f1-score
...
index_dot['rev_metric4'] = index['rev_metric4']
...
for name in names:
    ...
    if name=='rev_metric4' :
        ...
        clf4.fit(train_X, train_y)
        test_pred = clf4.predict_proba(test_X)[:,1]
