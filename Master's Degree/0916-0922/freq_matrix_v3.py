...
r_tf_tc1 = np.sum(key_vector,axis=0)/len(key_vector)
r_tf_tc2 = np.sum(non_key_vector,axis=0)/len(non_key_vector)
A = (np.sum(key_vector) +  np.sum(non_key_vector)) / 2

# baseline Method & proposed method1,2
...
rev_metric2 = soykeyword_metric2 * freq_metric3
rev_metric3 = (r_tf_tc1 - r_tf_tc2) * np.log (1+ A / ((r_tf_tc1+r_tf_tc2)+1e-8))
rev_metric4 = -(r_tf_tc1 - r_tf_tc2) * np.log (1+ A / ((r_tf_tc1+r_tf_tc2)+1e-8))
...
data_metric = {'keywords':words,
               'rev_metric1':rev_metric1, 'rev_metric2':rev_metric2, 'rev_metric3':rev_metric3, 'rev_metric4':rev_metric4
               }
...
names = ['rev_metric1', 'rev_metric2', 'rev_metric3','rev_metric4']
for name in names:
