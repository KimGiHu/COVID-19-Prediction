...
# # concatenated datas for augmentation
# for split in ['train', 'test']:
#     for N in Ns :
#         tmp = [] 
#         for name in names :
...
for name in names:
    ...
    for N in Ns:
        ...
        for (train_ids, test_ids) in kfold.split(train_dataset[0]):
            ...
            train_pred_resampled = clf.predict(train_X_resampled)
            ...
            print("Traing"+str(i+1)+' th')
            print(confusion_matrix(train_y_resampled, train_pred_resampled,labels=[0, 1, 2])) 
            print(classification_report(train_y_resampled, train_pred_resampled, labels=[0, 1, 2]))
            
            print("Test"+str(i+1)+' th')
