# 2023-09-12 / Epsilon-Support Vector Regression + Feature Selection
import pandas as pd
...
for name in names:
    ...
    for N in Ns:
        ...
        for (train_ids, test_ids) in kfold.split(train_dataset[0]):
            ...
            # clf = make_pipeline(StandardScaler(), SelectFromModel(LinearSVR(C=0.01, dual='auto')), SVR(kernel='rbf', C=1))
            # clf = make_pipeline(StandardScaler(), SVR(kernel='rbf', C=1))
            clf = make_pipeline(StandardScaler(), SelectFromModel(ExtraTreesRegressor(n_estimators=50)), SVR(kernel='rbf', C=1))
